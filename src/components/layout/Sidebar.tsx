...
function MonitoringNavSection({ collapsed }: { collapsed: boolean }) {
  const view = useUIStore((s) => s.view)
  const setView = useUIStore((s) => s.setView)
  const setMobileOpen = useUIStore((s) => s.setMobileSidebarOpen)
  ...
  return (
    <div className="mb-2 mt-4">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className={classNames(
          'flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-sm transition border',
          isMonitoringActive
            ? 'bg-pertamina-red-50 text-pertamina-red border-pertamina-red/20'
            : 'text-ink-secondary hover:text-ink-primary hover:bg-black/[0.04] border-transparent',
        )}
      >
        <span
          className={classNames(
            'grid h-7 w-7 shrink-0 place-items-center rounded-md',
            isMonitoringActive ? 'bg-pertamina-red/15 text-pertamina-red' : 'text-ink-secondary',
          )}
        >
          <Activity size={16} />
        </span>
