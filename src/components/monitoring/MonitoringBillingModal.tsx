...
import type { BillingDocumentType, BillingDocumentStatus } from '../../types/monitoring'

const DOC_TYPES: BillingDocumentType[] = ['BAP', 'BAPP', 'BAST', 'Invoice', 'Supporting Document']
...
export function MonitoringBillingModal({ open, onClose, mode, projectId, billingId }: Props) {
  ...
  const [docType, setDocType] = useState<BillingDocumentType>('BAP')
  ...
  useEffect(() => {
    if (mode === 'edit' && existing) {
      ...
      setDocType('BAP'); setPic(''); setTargetDate(''); setActualDate('')
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <label className="block">
            <span className="mb-1.5 block text-xs font-medium text-ink-secondary">Jenis Dokumen *</span>
            <select value={docType} onChange={(e) => setDocType(e.target.value as BillingDocumentType)} className="input-base appearance-none cursor-pointer pr-8">
